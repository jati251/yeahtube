...
function buildPages(page: number, totalPages: number, maxVisible: number): (number | "ellipsis")[] {
  ...
  const sideItems = Math.floor((maxVisible - 3) / 2); // items on each side of current page
...
export function PaginationControls({
  ...
  const btnInactive = "border-transparent text-zinc-600 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-900";
  ...
  const navBtn = "border-zinc-200/80 bg-white text-zinc-700 hover:bg-zinc-50 dark:border-zinc-800/80 dark:bg-zinc-950 dark:text-zinc-300 dark:hover:bg-zinc-900";
    ...
    <div className="my-6 flex flex-col items-center gap-2">
      ...
      <div className="flex items-center justify-center gap-1 sm:gap-1.5">
        ...
        <button onClick={onFirst} disabled={isFirst || loading}
          className={clsx(btnBase, "h-9 w-9", isFirst || loading ? btnDisabled : navBtn)}
        ...
        <button onClick={onPrev} disabled={isFirst || loading}
          className={clsx(btnBase, "h-9 w-auto px-3 gap-1", isFirst || loading ? btnDisabled : navBtn)}
          aria-label="Previous page">
          <ChevronLeft className="h-4 w-4" />
          <span className="hidden sm:inline">Prev</span>
        </button>

        {/* Page numbers — active page stays centered */}
        <div className="flex items-center gap-1">
          {pages.map((p, i) =>
            p === "ellipsis" ? (
              <span key={`e-${i}`} className="w-9 text-center text-sm text-zinc-400 dark:text-zinc-500">…</span>
            ) : (
              <button
                key={`p-${p}`}
                onClick={() => onPage?.(p)}
                disabled={loading}
                className={clsx(btnBase, "h-9 w-9 text-sm", p === page ? btnActive : btnInactive, loading && "cursor-not-allowed opacity-50")}
              >
                {p}
              </button>
            )
        ...
        <button onClick={onNext} disabled={isLast || loading}
          className={clsx(btnBase, "h-9 w-auto px-3 gap-1", isLast || loading ? btnDisabled : navBtn)}
        ...
        <button onClick={onLast} disabled={isLast || loading}
          className={clsx(btnBase, "h-9 w-9", isLast || loading ? btnDisabled : navBtn)}
